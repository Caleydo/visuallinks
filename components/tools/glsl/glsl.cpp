...
//----------------------------------------------------------------------------- 
//now directly set in the GLSL source
/*
...
GLint glShader::GetUniformLocation(const GLcharARB *name)
{
	...
	uniformsmap_t::iterator found = _uniforms.find(std::string(name));
	...
	else
	{
	  ...
	  //else
	  _uniforms.insert(uniformsmap_t::value_type(std::string(name), loc));	  
	}
	return loc;
}
